...
def find_top_n_similar_embeddings(new_text, existing_embeddings, n=10):
    ...
    similarities = [cosine_similarity(new_embedding, existing_embedding) for existing_embedding in existing_embeddings]
    top_n_indices = np.argsort(similarities)[-n:][::-1]  # Get indices of the top n similarities in descending order
    top_n_embeddings = [existing_embeddings[i] for i in top_n_indices]
    top_n_scores = [similarities[i] for i in top_n_indices]
    return top_n_embeddings, top_n_indices, top_n_scores  # Return top n embeddings, indices, and scores

# Load the existing embeddings from the CSV
embeddings_output_file_path = 'embeddings_ADMISSIONS.csv'
existing_embeddings = pd.read_csv(embeddings_output_file_path).values.tolist()  # Load embeddings
...
def get_top_10_closest(new_text: str):
    closest_embeddings, indexes, similarity_scores = find_top_n_similar_embeddings(new_text, existing_embeddings)
    top_10_closest_rows = [concatenated_rows[i-1] for i in indexes]
    result = [{"row": row, "similarity_score": score} for row, score in zip(top_10_closest_rows, similarity_scores)]
